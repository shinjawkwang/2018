...
def is_duplicated_number(three_digit):
    ...
    # ===Modify codes below=============
    # 조건에 따라 변환되어야 할 결과를 result 변수에 할당
    f_letter = three_digit[0]
    s_letter = three_digit[1]
    t_letter = three_digit[2]
    if(f_letter == s_letter or f_letter == t_letter or s_letter == t_letter):
        result = True
    else:
        result = False
    # ==================================
...
def get_not_duplicated_three_digit_number():
    ...
    tester = str(result)
    while(not is_validated_number(tester)):
        result = get_random_number()
        tester = str(result)
    # ==================================
    return result
...
def get_strikes_or_ball(user_input_number, random_number):
    ...
    balls = 0
    for i in range(3):
        f_case = (3-1+i) % 3    # 0~2에서 나머지 두 숫자를 반환
        s_case = (3-2+i) % 3    # 0 -> 2,1 // 1 -> 0,2 // 2 -> 1,0
        if(user_input_number[i] == random_number[i]):
            strikes += 1
        elif(user_input_number[i] == random_number[f_case] or user_input_number[i] == random_number[s_case]):
            balls += 1
...
def main():
    ...
    flag = False    # 성공 후 다시 게임을 진행할 때 필요
    b_flag = False  # 성공 후 no 입력시 반복문 탈출을 위함
    while(True):
        if(flag):
            random_number = str(get_not_duplicated_three_digit_number())
            print("Random Number is : ", random_number)
